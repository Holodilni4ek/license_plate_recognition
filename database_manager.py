...
class DatabaseManager:
    ...
    def __init__(self):
        ...
        if not all(self.db_config.values()):
            ...
            raise ValueError(f"Missing database configuration variables: {missing_vars}")
    ...
    def is_vehicle_registered(self, vehicle_number: str) -> bool:
        ...
        query = "SELECT 1 FROM vehicle WHERE vehiclemark = %s LIMIT 1"
        result = self.execute_query(query, (vehicle_number,))
        return len(result) > 0
    
    def get_registered_vehicles(self) -> set:
        """Get all registered vehicle numbers."""
        query = "SELECT vehiclemark FROM vehicle"
        result = self.execute_query(query)
        return {row[0] for row in result}
    
    def add_driver(self, firstname: str, secondname: str, patronymic: str, 
                   birthdate: str, nationality: str) -> Optional[int]:
        ...
        """
        try:
            return self.execute_insert(query, (firstname, secondname, patronymic, birthdate, nationality))
        except Exception as e:
            logging.error(f"Failed to add driver: {e}")
            return None
    
    def add_vehicle(self, vehicle_mark: str, vehicle_color: str, vehicle_type: str, 
                   driver_id: int) -> Optional[int]:
        """Add a new vehicle to the database."""
        query = """
        INSERT INTO vehicle (vehiclemark, vehiclecolor, vehicletype, id_driver) 
        VALUES (%s, %s, %s, %s) 
        RETURNING id_vehicle
        """
        try:
            return self.execute_insert(query, (vehicle_mark, vehicle_color, vehicle_type, driver_id))
        except Exception as e:
